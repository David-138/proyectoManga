        ...
        data.mangas.forEach((manga) => {
          if (manga.ID_Portada) {
            fetch(`https://kitsu.io/api/edge/manga/${manga.ID_Portada}`)
              ...
              .catch((error) => console.error("Error al cargar portada:", error));
          }
        });
      })
          ...
          {mangas.map((manga) => (
            <li key={manga.ID_manga}>
              <div className="mangaSelect">
                <Link className="portadas" to={`/manga/${manga.ID_manga}`} state={{ fromPage: page }}>
                  <img
                    src={coverImages[manga.ID_manga] || "/placeholder.jpg"}
                    alt={manga.titulo || manga.title}
                  />
                </Link>
                <Link to={`/manga/${manga.ID_manga}`} state={{ fromPage: page }}>
